            ...
            if len(globalPersonData)==0:
                ...
                for sclique in Allcliques:
                    ...
                    for i in range(1,len(sclique)):
                        ...
                        globalPersonData[len(globalPersonData)-1].histogram_h=np.minimum(cameras[localgloballink[sclique[i]-1][1]].PersonData[localgloballink[sclique[i]-1][2]].histogram_h,globalPersonData[len(globalPersonData)-1].histogram_h)
            else:
                for k in range(len(cameras)):
                    ...
                    for i in range(len(cameras[k].PersonData)):
                        ...
                        for j in range(len(globalPersonData)):
                            if(cameras[k].PersonData[i].globalFoundOutPersonIndex==j):
                                globalHungarian[len(globalHungarian)-1].append(np.sum(np.absolute(np.subtract(cameras[k].PersonData[i].histogram_h,globalPersonData[j].histogram_h)))-0.2)
                            else:
                                globalHungarian[len(globalHungarian)-1].append(np.sum(np.absolute(np.subtract(cameras[k].PersonData[i].histogram_h,globalPersonData[j].histogram_h))))
                    for i in range(len(cameras[k].PersonData)):
                        for j in range(len(globalPersonData)):
                    ...
                    if(len(globalHungarian)!=0):
                        ...
                        for pos in range(len(row_ind)):
                            if(globalHungarian[row_ind[pos]][col_ind[pos]]<0.5):
                                cameras[k].PersonData[row_ind[pos]].globalFoundOutPersonIndex=col_ind[pos]
                                #globalHungarian[col_ind[pos]].histogram_h=np.minimum(globalHungarian[col_ind[pos]].histogram_h,cameras[k].PersonData[row_ind[pos]])
